...
//5)Dada una cadena, extraer la cuarta y quinta letra usando el método substring.
        ...
        System.out.println("la cuarta y quinta letra de la palabra "+cadena1+ " son: " + cadena1.substring(3, 5));// no sé por qué con el 4 no muestra la i pero con el 5 si
        System.out.println("-----------------------------------------------------------------------------------------");
...
//9)Buscar un elemento dentro de un vector que nosotros le pedimos por teclado. Indicar las posiciones donde se encuentra.
        ...
        for (int i = 0; i < vector.length; i++) {
            if (vector[i] == (busqueda1)) {
                System.out.println("Se encontró el numero ingresado y esta en la posición: " + i);
                encontro = true;
            }
        }
        if (encontro == false) {
